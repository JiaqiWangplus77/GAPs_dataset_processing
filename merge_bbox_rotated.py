...
image_num = 17  # raise error when out of range
bbox_padding = 0  
...
def draw_contours(img,contours,thickness):
    ...
    for i in range(len(contours)):
        ...
        cv2.drawContours(img,[min_rect],0,color=[255,255,255],thickness=-1)

#        plt.figure()
#        plt.imshow(img) 
#        plt.title('draw contours'+str(min_rect))  
        bbox.append(min_rect)
    return img,bbox
...
#        obj_class = np.unique(obj[:,0])
        ...
        for item in range(len(obj_class)):
            ...
            # find the rectangle to cover the contours
                ...
                img, bbox = draw_contours(img,contours,-1)           
            ...
            for num in range(len(bbox)):                
                ...
                #print(rect)
                cv2.drawContours(img,[coordinate],0,color=[255,0,0],thickness=5)               
                xc = (coordinate[0,0] + coordinate[2,0] ) /2
                yc = (coordinate[0,1] + coordinate[2,1] ) /2
                
                v1 = coordinate[0]-coordinate[1]
                v2 = coordinate[0]-coordinate[2]
                ...
                if v_height[1] == 0:
                    ...
                    angle = math.atan(v_height[1] / v_height[0])
                

           
                rect = [xc,yc,width,height,angle]
